...
def create_app(config: dict = None, validate_config: bool = True):
    ...
    if not debug_mode and secret_key in insecure_keys:
        ...
        print(
            "Set WPGEN_SECRET_KEY environment variable to a secure random value.", file=sys.stderr
        )
        print(
    ...
    if debug_mode and not secret_key:
        ...
        print(
            "⚠ Warning: Using development secret key. Set WPGEN_SECRET_KEY for production.",
            file=sys.stderr,
        )
    ...
    def handle_exception(error):
        ...
        if isinstance(error, HTTPException):
            return (
                jsonify({"code": error.code, "message": error.name, "details": error.description}),
                error.code,
            )
    ...
    def version():
        ...
        return jsonify({"version": __version__, "app": "wpgen-web"})
    ...
    def api_config_validate():
        ...
        try:
            ...
            if config_to_validate:
                ...
                return (
                    jsonify({"ok": True, "message": "Configuration is valid", "config": summary}),
                    200,
                )
            else:
                ...
                return (
                    jsonify(
                        {"ok": True, "message": "Configuration file is valid", "config": summary}
                    ),
                    200,
                )
            ...
            return (
                jsonify(
                    {"ok": False, "message": "Configuration validation failed", "errors": errors}
                ),
                400,
            )
        except Exception as e:
            logger.error(f"Config validation error: {e}")
            return jsonify({"ok": False, "message": f"Validation error: {str(e)}"}), 500
