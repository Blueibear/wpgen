...
"""
        ...
        assert any("WP_DEBUG" in p for p in violation_patterns), "Should detect WP_DEBUG"
        assert any(
            "error_reporting" in p for p in violation_patterns
        ), "Should detect error_reporting"
        assert any(
            "display_errors" in p for p in violation_patterns
...
"""
        ...
        assert results["valid"], "wp-config-sample.php should be exempt from WP_DEBUG checks"
        assert len(results["violations"]) == 0, "Should not flag wp-config-sample.php"
