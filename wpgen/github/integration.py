...
"""
            ...
            try:
                ...
                # Try push with GitPython first
                try:
                    origin.push(refspec=f"{branch}:{branch}")
                    logger.info(f"Pushed to GitHub: {branch} branch")
                except Exception as push_error:
                    # If GitPython fails, try command-line git as fallback
                    logger.warning(f"GitPython push failed: {push_error}")
                    logger.info("Attempting push via command-line git...")
                    ...
                    if result.returncode != 0:
                        logger.error(f"Command-line git push failed: {result.stderr}")
                        raise Exception(f"Push failed: {result.stderr}")

                    logger.info(f"Successfully pushed via command-line git: {branch} branch")
