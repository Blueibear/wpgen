...
class GitHubIntegration:
    ...
    def get_repository(self, name: str, owner: str | None = None) -> dict[str, Any]:
    ...
    def push_to_github(
        ...
        try:
            ...
            # Use HTTPS URL without embedded credentials (secure)
...
"""
            ...
            # Push to GitHub using secure credentials
            try:
                # Configure git for larger buffers (helps with HTTP 500 errors)
                with repo.config_writer() as config:
                    config.set_value("http", "postBuffer", "524288000")  # 500MB
                    config.set_value("http", "version", "HTTP/1.1")
                logger.info("Configured git for large file push")
                ...
                # Use secure credential helper for push
                logger.info("Pushing with secure credential helper (GIT_ASKPASS)")

                with SecureCredentialHelper(self.token) as cred_helper:
                    # Get environment with GIT_ASKPASS configured
                    git_env = cred_helper.build_askpass_env()

                    # Push using command-line git with secure env
                    ...
                    logger.debug("Credentials were provided securely via GIT_ASKPASS")

            except Exception as e:
                logger.error(f"Failed to push to GitHub: {str(e)}")
                # Provide helpful error message
                if "HTTP 500" in str(e) or "RPC failed" in str(e):
                    logger.error(
                        "GitHub push failed with HTTP 500. This can happen due to:\n"
                        "  - Large repository size\n"
                        "  - Network issues\n"
                        "  - GitHub server temporary issues\n"
                        "Suggestion: Try again in a few moments, or disable 'Push to GitHub' to test locally."
                    )
                raise
